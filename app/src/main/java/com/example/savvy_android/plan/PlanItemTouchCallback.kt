package com.example.savvy_android.plan
...
class PlanItemTouchCallback : ItemTouchHelper.SimpleCallback(
    ...
    private fun getViewWidth(viewHolder: RecyclerView.ViewHolder): Float {
        val viewWidth =
            (viewHolder as PlanListAdapter.PlanViewHolder).itemView.findViewById<ConstraintLayout>(
    ...
    override fun onSelectedChanged(viewHolder: RecyclerView.ViewHolder?, actionState: Int) {
        if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE) {
            ...
            if (PlanListAdapter.hasSwipe) {
                viewHolder?.let { vh ->
                    val recyclerView = vh.itemView.parent as? RecyclerView
                    recyclerView?.let {
                        PlanListAdapter.resetHideX(
                            PlanListAdapter.hasSwipePosition,
                            recyclerView
                        )
                    }
                }
    ...
    override fun onChildDraw(
        ...
        if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE) {
            val view = (viewHolder as PlanListAdapter.PlanViewHolder).hideX
    ...
    override fun clearView(
        ...
        val view = (viewHolder as PlanListAdapter.PlanViewHolder).hideO

        view.isClickable = currentDx == -clamp
        if (view.isClickable) {
            PlanListAdapter.hasSwipe = true   // swipe 가능한 아이템 존재
            PlanListAdapter.hasSwipePosition = viewHolder.adapterPosition // 해당 아이템 위치
        }
    }
    ...
    private fun getView(viewHolder: RecyclerView.ViewHolder): View {
        return (viewHolder as PlanListAdapter.PlanViewHolder).itemView.findViewById(R.id.item_plan_hide_x)
    }
